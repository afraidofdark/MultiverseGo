/*
 * Copyright (c) 2019-2026 OtSoftware
 * This code is licensed under the GNU Lesser General Public License v3.0 (LGPL-3.0).
 * For more information, including options for a more permissive commercial license,
 * please visit [otsoftware.tr] or contact us at [info@otsoftare.tr].
 */

// Linux platform helpers for the standalone game executable. Mirrors the
// Windows/Web/Android variants in this folder; the helpers are portable
// (SDL window flags + resource paths relative to the CWD + a plain while-loop
// main loop), so the body is identical to windows_main.h.

#include "../Codes/Game.h"
#include "Logger.h"
#include "Util.h"

#include <stdio.h>

#include <chrono>
#include <iostream>

#define PLATFORM_SDL_FLAGS (SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_SHOWN)

namespace ToolKit
{
  inline void PlatformPreInit(Main* g_proxy)
  {
    g_proxy->m_resourceRoot = ConcatPaths({".", "..", "Resources"});
    g_proxy->m_cfgPath      = ConcatPaths({".", "..", "Config"});

    GetLogger()->SetWriteConsoleFn([](LogType lt, String ms) -> void { std::cout << ms << std::endl; });
  }

  inline void PlatformMainLoop(bool* running, void (*TK_Loop)())
  {
    while (*running)
    {
      TK_Loop();
    }
  }

  inline void PlatformAdjustEngineSettings(int availableWidth, int availableHeight, EngineSettings* engineSettings) {}
} // namespace ToolKit
